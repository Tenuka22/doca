...
log.header(f"{DATASET.upper()} - XGBoost Optimization for all SEQ_LENS")
...
log.info(f"Train samples: {len(y_train)}  Val: {len(y_val)}  Test: {len(y_test)}")
log.info(f"Classes: {nc}  Weights: {class_weight_dict}")

for SEQ_LEN in SEQ_LENS:
    STEP = max(1, SEQ_LEN // 3)
    SUBDIR = f"seq_len_{SEQ_LEN}"

    log.header(f"Optimizing SEQ_LEN={SEQ_LEN}, STEP={STEP}")
    ...
    log.info(f"Best val acc for SEQ_LEN={SEQ_LEN}: {study.best_value:.4f}")
    ...
    log.ok(f"Params saved to {best_params_dir / 'best_params.json'}")
    for key, value in best.items():
        log.detail(f"{key}: {value}")

log.header("All SEQ_LENS complete. Summary:")
for SEQ_LEN in SEQ_LENS:
    log.detail(f"SEQ_LEN={SEQ_LEN}: artifacts/{DATASET}/xgboost/seq_len_{SEQ_LEN}/best_params.json")
