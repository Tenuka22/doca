...
export default function SpriteScreen() {
    ...
    orpc.feedSprite.mutationOptions({
      onSuccess: (data) => {
        ...
        queryClient.invalidateQueries({
          queryKey: orpc.getSpriteCollection.key(),
        });
        queryClient.invalidateQueries({
  ...
  const LEVELS = useMemo(
    () => [
      ...
      { name: "Diamond", min: 10_000, color: colors.primary },
    ],
    [colors]
  );
  ...
  const totalPossibleItems = useMemo(
    () =>
      Object.values(catalog).reduce(
        (acc: number, val: any) => acc + val.length,
        0
      ),
    [catalog]
  );
  ...
  const currentLevel = useMemo(
    () => [...LEVELS].reverse().find((l) => totalEarned >= l.min) ?? LEVELS[0],
    [totalEarned, LEVELS]
  );
    ...
    <>
        ...
        >
          ...
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center gap-3">
              ...
              <View>
                ...
                <Text className="font-black font-sans text-foreground text-xl tracking-tight">
                  {currentLevel?.name}
                </Text>
              </View>
            </View>
            <View className="flex-row items-center gap-2 rounded-full border border-warning/20 bg-warning/10 px-4 py-2">
              <Moon color={colors.warning} size={16} />
              <Text className="font-black font-sans text-lg text-warning">
                {credits?.balance ?? 0}
              </Text>
            </View>
          </View>
          ...
          <View className="gap-2">
            <View className="flex-row items-center justify-between px-1">
              <Text className="font-bold font-sans text-[10px] text-muted-foreground uppercase tracking-widest">
                Progress to {nextLevel?.name ?? "Max"}
              </Text>
              <Text className="font-bold font-sans text-[10px] text-primary">
          ...
          <View className="relative items-center justify-center py-4">
              ...
              >
                <View
                  className={`h-20 w-20 items-center justify-center rounded-3xl border-4 shadow-xl ${
                    droppedItem.rarity === "legendary"
                      ? "border-primary bg-primary/10"
                      : droppedItem.rarity === "rare"
                ...
                >
                  {(() => {
                    const Icon =
                      ICON_COMPONENTS[droppedItem.iconName] ?? Sparkles;
                    return (
                      <Icon
                        color={
                          droppedItem.rarity === "legendary"
                ...
                <View className="rounded-full bg-foreground px-4 py-1">
                  <Text className="font-black font-sans text-[10px] text-background uppercase tracking-widest">
            ...
            <Pressable
              className="z-10 h-72 w-72 items-center justify-center"
            ...
            {/* Status Overlays */}
            <View className="absolute right-4 bottom-0 z-20 rounded-2xl border-2 border-border bg-card p-3 shadow-sm">
            ...
            <View className="absolute bottom-0 left-4 z-20 rounded-2xl border-2 border-border bg-card p-3 shadow-sm">
              <View className="flex-row items-center gap-2">
          ...
          <View className="gap-3 rounded-card border-2 border-border bg-card p-4 shadow-sm">
            ...
            <View className="gap-4 py-2">
              ...
              <View className="gap-2">
                <View className="flex-row items-center justify-between">
                  ...
                  <Text className="font-black font-sans text-[10px] text-foreground">
                    {uniqueCollected}/{totalPossibleItems}
                  </Text>
                </View>
                <View className="h-1.5 overflow-hidden rounded-full bg-muted">
                  <View
                    ...
                    style={{
                      width: `${totalPossibleItems > 0 ? (uniqueCollected / totalPossibleItems) * 100 : 0}%`,
                    }}
                  />
                </View>
              </View>
            ...
            <View className="mt-1 flex-row gap-2">
              <View className="flex-1 items-center rounded-control border-2 border-border bg-background px-3 py-3">
                <Text className="font-bold font-sans text-[8px] text-muted-foreground uppercase tracking-widest">
                  Streak
                </Text>
                <Text className="mt-1 font-black font-sans text-foreground text-xl">
                  {sprite?.streakDays ?? 0}
                </Text>
              </View>
              <View className="flex-1 items-center rounded-control border-2 border-border bg-background px-3 py-3">
          ...
          <Pressable
            className="overflow-hidden rounded-card border-2 border-border bg-card p-5 shadow-sm"
            onPress={() => router.push("/sprite/trophies")}
          >
            <View className="absolute -right-6 -bottom-6 rotate-12 opacity-5">
              <Trophy color={colors.foreground} size={100} />
            </View>
            <View className="flex-row items-center justify-between">
          ...
          <View className="gap-4">
            ...
            >
              {inventoryQuery.data?.filter((i) => i.quantity > 0).length ===
                0 && (
                <View className="w-full items-center justify-center rounded-2xl border-2 border-border border-dashed bg-card p-6">
                  <Text className="text-center font-bold font-sans text-muted-foreground text-xs">
                    Your bag is empty! Go to the shop.
                  </Text>
                </View>
              )}
              {inventoryQuery.data
                ...
                .map((item) => {
                  ...
                  return (
                    <Pressable
                      className="w-24 items-center gap-2 rounded-2xl border-2 border-border bg-card p-4 shadow-sm active:bg-muted/30"
                    ...
                    >
                      ...
                      <Text
                        className="text-center font-bold font-sans text-[10px] text-foreground uppercase"
                        numberOfLines={1}
                      >
                        {item.itemId}
                      </Text>
                      <View className="rounded-full bg-primary px-2 py-0.5">
                        <Text className="font-black text-[8px] text-white">
                          x{item.quantity}
                        </Text>
                      </View>
                    </Pressable>
          ...
          <View className="rounded-card border-2 border-border bg-card p-5 shadow-sm">
            ...
            <View className="flex-row gap-4">
              <View className="flex-1 items-center gap-1 rounded-2xl bg-muted/30 p-4">
              ...
              <View className="flex-1 items-center gap-1 rounded-2xl border border-primary/20 bg-primary/10 p-4">
                <Text className="font-bold font-sans text-[10px] text-primary uppercase">
                  You
                </Text>
                <Text className="font-black font-sans text-primary text-xl">
                  200
                </Text>
              </View>
            </View>
          ...
          {/* Level Path Card */}
          <View className="overflow-hidden rounded-card border-2 border-primary/20 bg-primary/5 p-5 pb-10 shadow-sm">
            ...
            <View className="relative flex-row items-center justify-between px-2">
              <View className="absolute top-4 right-4 left-4 h-0.5 bg-muted" />
              {[1, 2, 3, 4, 5].map((l) => {
                const isCurrent = l === LEVELS.indexOf(currentLevel!) + 1;
                const isPast = l < LEVELS.indexOf(currentLevel!) + 1;
                return (
                  <View className="items-center gap-1" key={l}>
                    <View
                    ...
                    >
                      <Text
                        className={`font-black text-[10px] ${isCurrent ? "text-white" : "text-muted-foreground"}`}
                      >
                        {l}
                      </Text>
                    </View>
            ...
            <Text className="mt-4 text-center font-bold font-sans text-[10px] text-primary uppercase tracking-wider">
      ...
      <ScreenBottomBar>
        ...
        >
          ...
          <Text className="font-black font-sans text-foreground text-xs uppercase">
            Shop
          </Text>
        </Pressable>
        <Pressable
