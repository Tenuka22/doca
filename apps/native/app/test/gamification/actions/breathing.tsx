...
export default function BreathingExerciseScreen() {
  ...
  useEffect(() => {
    ...
    const startValue = currentPhase === "inhale" ? 0 : currentPhase === "hold" ? 1 : currentPhase === "exhale" ? 1 : 0.15;
    const endValue = currentPhase === "inhale" ? 1 : currentPhase === "hold" ? 1 : currentPhase === "exhale" ? 0 : 0;
        ...
        <View className="overflow-hidden rounded-card border-2 border-border bg-card">
          <View className="gap-4 border-border border-b-2 px-card py-card">
            ...
            <Text className="max-w-[28rem] font-normal font-sans text-base text-muted-foreground leading-6">
              A guided breathing loop with stronger feedback, live phase pacing, and adjustable rhythm.
            </Text>
          </View>
          ...
          <View className="gap-4 px-card py-card">
                  ...
                  <Animated.View
                    ...
                    style={{
                      backgroundColor: fill.interpolate({
                        inputRange: [0, 0.15, 1],
                        outputRange: [colors.background, colors.primary, colors.secondary],
                      }),
                      transform: [
                        {
            ...
            <View className="flex-row gap-2">
              <Button className="flex-1" onPress={handleStart} variant="primary" disabled={running}>
                Start
              </Button>
              <Button className="flex-1" onPress={handleStop} variant="secondary">
                Reset
              </Button>
            </View>
            ...
            <Button className="w-full" href="/test/gamification/actions" variant="secondary">
              Back to Actions ›
            </Button>
          </View>
        </View>
