...
function TenantAttendancePage() {
  ...
  const selectedDoctorWindows = useMemo(() => {
    if (!hospitalBlocks?.recurringWindows) return [];
  ...
  const isWithinWindow = (timestamp: string): boolean => {
    if (!hospitalBlocks?.recurringWindows || selectedDoctorWindows.length === 0)
      return true;
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog onOpenChange={setDialogOpen} open={dialogOpen}>
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>
                {editingEvent ? "Edit Attendance Event" : "Log Attendance Event"}
              </DialogTitle>
              <DialogDescription>
                {editingEvent
                ...
                <div className="rounded-lg border border-border/50 bg-primary/5 p-3">
                  ...
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {selectedDoctorWindows.map((w, i) => (
                      <Badge className="text-[10px]" key={i} variant="secondary">
                        {w}
                      </Badge>
                    ))}
                  </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="flex flex-col gap-1">
          ...
          <Popover>
            <PopoverTrigger
              render={<Button className="w-[240px] justify-start text-left font-normal" variant="outline" />}
            >
      ...
      {isLoading ? (
        ...
        <Card className="flex flex-col items-center justify-center py-12">
          ...
          <CardTitle className="mt-3 text-base">No events for this date</CardTitle>
          <CardDescription>
