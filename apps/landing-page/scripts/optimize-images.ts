...
async function optimizeImages() {
  ...
  console.log("🖼️  Original:", metadata.width, "x", metadata.height, metadata.format, `${(inputBuffer.length / 1024).toFixed(0)}KB`);
  ...
  for (const { width, suffix } of sizes) {
    ...
    await sharp(inputBuffer).resize(width).png({ compressionLevel: 9, palette: true }).toFile(`${base}.png`);
    const pngSize = (await readFile(`${base}.png`)).length / 1024;

    await sharp(inputBuffer).resize(width).webp({ quality: 80, effort: 6 }).toFile(`${base}.webp`);
    const webpSize = (await readFile(`${base}.webp`)).length / 1024;

    await sharp(inputBuffer).resize(width).avif({ quality: 65, effort: 6 }).toFile(`${base}.avif`);
    const avifSize = (await readFile(`${base}.avif`)).length / 1024;

    console.log(`  ${width}px: PNG=${pngSize.toFixed(0)}KB WebP=${webpSize.toFixed(0)}KB AVIF=${avifSize.toFixed(0)}KB`);
  }

  // Blur placeholder
  const blurBuf = await sharp(inputBuffer).resize(20).blur(3).png({ compressionLevel: 9 }).toBuffer();
  const blurB64 = blurBuf.toString("base64");
  await writeFile(join(cacheDir, "blur.txt"), `data:image/png;base64,${blurB64}`);
  console.log(`  blur: ${(blurBuf.length / 1024).toFixed(0)}KB`);

  // Optimize source in place
  const tmpPath = `${inputPath}.tmp`;
  await sharp(inputBuffer).png({ compressionLevel: 9, palette: true }).toFile(tmpPath);
